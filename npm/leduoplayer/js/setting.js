...
var leduo = {
	...
	'hideLoading': function() {
		var css ='<style type="text/css">';css+='#loading-box{display: none;}';css+='</style>';$('body').append(css).addClass("");
	},
	'start': function() {
		$.ajax({
			url: "/dmku/api.php",
	...
	'load': function() {
		...
		if (((leduo.versions.ios || leduo.versions.iPad) && leduo.versions.weixin) || ((leduo.versions.ios || leduo.versions.iPad) && leduo.versions.uc) || ((leduo.versions.ios || leduo.versions.iPad) && leduo.versions.webKit)) {setTimeout(() => {leduo.loadedmetadataHandler()}, 1500);};
		leduo.danmu.send();
		leduo.danmu.list();
		leduo.def();
		leduo.video.try();
	...
	'video': {
		...
		'con_play': function() {
			...
			var cplayer =
				`<div class="memory-play-wrap"><div class="memory-play"><span class="close">×</span><span>上次看到 </span><span>${leduo.ctime}</span><span class="play-jump">跳转播放</span></div></div>`
	...
	'jump': {
		...
		'head': function() {
			if (leduo.stime > leduo.playtime) leduo.playtime = leduo.stime;
			if (leduo.frists == 1) {
			...
			if (config.next != '') {
				if (leduo.lasts == 1) {
					setInterval(function() {
						...
						if (e < leduo.last_tip) leduo.dp.notice('即将为您跳过片尾');
					...
					success: function(d) {
						if (d.code == 23) {
							...
							$(a).each(function(index, item) {
								l =
									`<d class="danmuku-list" time="${item[0]}"><li>${leduo.formatTime(item[0])}</li><li title="${item[4]}">${item[4]}</li><li title="用户：${item[3]}  IP地址：${item[5]}">${item[6]}</li><li class="report" onclick="leduo.danmu.report(\'${item[5]}\',\'${b}\',\'${item[4]}\',\'${item[3]}\')">举报</li></d>`
								$(".list-show").append(l);
							})
						}
